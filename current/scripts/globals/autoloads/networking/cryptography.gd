...
func save_message(sender_identity: String, message_id: int, payload: PackedByteArray) -> void:
...
func send_key(ID: int, this_target: String = "0") -> void:
	Networking.send_message_to_user({"type": "encrypted_key", "message_id": ID, "key": encrypted_messages_sent[ID]["key"].save_to_string()}, this_target)
...
func decode_message(sender_identity: String, message_id: int, key: String) -> void:
	if encrypted_messages_recieved.has(sender_identity) and encrypted_messages_recieved[sender_identity].has(message_id):
		...
		Networking.process_message(decrypted_message, sender_identity)
