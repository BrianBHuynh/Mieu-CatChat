...
static func get_stat(node_name: String) -> float:
	var cur_stat: float = Saves.get_value("status", node_name, 100.0)
	var time_changed: float = Saves.get_value("status", node_name + "_time", Time.get_unix_time_from_system())
	var rate_of_change: float = Saves.get_value("status", node_name + "_rate", 1.0)
	var val_max: float = Saves.get_value("status", node_name + "_max", 100.0)
	var val_min: float = Saves.get_value("status", node_name + "_min", 0.0)
	cur_stat = cur_stat + (get_time_difference_minutes(time_changed)*rate_of_change)
	if cur_stat <= val_min:
		Saves.set_value("vpet", node_name, val_min)
		Saves.set_value("vpet", node_name + "_time", Time.get_unix_time_from_system())
		return val_min
	elif cur_stat >= val_max:
		Saves.set_value("vpet", node_name, val_max)
		Saves.set_value("vpet", node_name + "_time", Time.get_unix_time_from_system())
		return val_max
	else:
		return cur_stat
...
static func set_stat(stat_name: String, value: float) -> float:
	var val_max: float = Saves.get_value("status", stat_name + "_max", 100.0)
	var val_min: float = Saves.get_value("status", stat_name + "_min", 0.0)
	if value <= val_min:
...
static func get_param(stat_name: String, param: String) -> bool:
	if param_exist(stat_name, param):
		return Saves.get_value("vpet", stat_name + "_" + param,  0.0)
	else:
		return 0.0
